...
function main(){
	...
	var tracking = false;
	const outlineElement = document.getElementsByClassName("outline")[0];

	function switchModelOutlineVisibility() {
		if(tracking){
			console.log("switching to outline");
			tracking = false;
			// outlineElement.classList.remove("hide");
			outlineElement.style.visibility = "visible";
			// outlineElement.classList.add("hide");
			// outlineElement.classList.add("hide");
			model.visible = false;
		} else {
			console.log("switching to 3d model");
			tracking = true;
			// outlineElement.classList.add("hide");
	...
	function onResults(results) {
		...
		if(results.poseLandmarks){
			...
			// var diff = now-preTime;

			// preTime = now;

			// var seconds = (diff/1000);

			// var updatesPerSecond = (1/seconds);
			if(!tracking){
				switchModelOutlineVisibility();
			}
			

			var max_length_memories = 5;

			// console.log("memoryLength",max_length_memories, "seconds:",seconds,"updates per second", updatesPerSecond);

			//update list of points either by adding to or shifting them

			if(pointsMemory.length<max_length_memories){
			...
			//Aggregate the points by doing a simple average

			var length_diff = Math.max(0,memoryLength - max_length_memories);

			if(memoryLength>1){

				for(var i = 0;i<smoothPoints.length;i++){
					// for(var j = 1;j<memoryLength;j++){
					for(var j = 1+length_diff;j<memoryLength;j++){
	...
	function onWindowResize(){
	    ...
	    //For mobile set mediaStream values on rotate, stops breakage of portrait mode;
