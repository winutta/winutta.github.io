...
window.onload = function() {
  // Get A WebGL context
  var canvasTest = document.getElementById("c2");
  var gl = canvasTest.getContext("webgl");
  if (!gl) {
    return;
  }
  ...
  function getExt(name, msg) {
    ...
    console.log((ext ? "can " : "can **NOT** ") + msg);
    return ext;
  }
  ...
  function test(format) {
    ...
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
		...
		renderGPU = false;
        console.log("can **NOT** render to " + glEnum(gl, format) + " texture");
        return;
    } else {
